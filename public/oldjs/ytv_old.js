                        ...
                        utils.ajax.get( utils.endpoints.userInfo(), function(userInfo){
                            ...
                            for(i=0; i<videos.length; i++){
                                if(videos[i].media$group.yt$duration){
                                    ...
                                    var time=video.duration; //console.log('duration', video.duration);
                                    var sec = time%60;
                                    var sec_fixed = (parseInt(sec/10)==0)? '0'+ sec : sec; //append 0 if sec is single digit
                                    var min = time/60;
                                    var min_fixed =parseInt(min); //truncate decimal
                                    var timeSlot = min_fixed + ' : ' + sec_fixed;
                                    ...
                                    list += '<div class="ytv-thumb"><div class="ytv-thumb-stroke"></div><span>'+(timeSlot)+'</span><img src="'+(video.thumb)+'"></div>';
                                    list += '<div class="ytv-content"><b>'+(video.title)+'</b>'; //console.log('view count: ',video.stats);
                                    if (video.stats)
                                    {
