...
function displayFeed(data, name) {
  ...
  if (data.query.results) {
    ...
    var i,title,link,desc,image,html_str,temp,imagetag,image,patt,res,image_html,pub_date,ago,output,isShown;
    ...
    for ( i = 0; i < no_items; i++) {
      ...
      if (name.substr(0, 8) == "BBC_News") {
       ...
       image_html = (image !== '') ? ("<img width='100' height='80' align='left' src='" + image + "'></img>") : '';
      ...
      if (name.substr(0, 11) == "Google_News") { //google news
         ...
         output = "<div class='entry entry-" + i + "'>" + desc + "<hr/></div>";

      } else {

        desc = desc.replace(/<(?:.|\n)*?>/gm, '');
         output = "<div class='yahoo_news_entry'><h3><a href='" + link + "' target='blank'>" + image_html + "<b>" + title + "</b></a></h3><p>" + desc + "</p><span class='time'>" + ago + "</span><hr/></div>";
      }
      $("#" + name).append(output);
    ...
    if (name.substr(0, 11) == "Google_News") { //fixing css
      ...
      $("div.lh .p").remove();
      //$("#"+name+" div.lh").append("<span class='time'>" + ago + "</span>");
